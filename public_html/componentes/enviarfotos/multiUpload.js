...
function multiUpload(id, filesdiv, options,gal_codigo)
{
	/**
	...
	{
		//html  = '<table><tr><td>Arquivos</td><td>Tamanho</td><td>Remover</td><td>Progresso</td></tr>';
		//html += '';
		//html += '';
		
		html  = '<div><div class="c30p bold sub_Titulo">Arquivo</div><div class="c100 ac bold sub_Titulo">Tamanho</div>';
		html += '<div class="c100 ac bold sub_Titulo">Remover</div><div class="c30p bold sub_Titulo">Progresso</div></div><div class="sep "></div>';
		html += '<div id="files_list" class="sub_Titulo"></div>';
	...
	this.onSelected = function(e)
	{
		for (var file in e.files)
		{
			...
			divfile.innerHTML = '<div class="c30p">'+info.name+'</div><div class="c100 ac">'+size(info.size)+'</div><div class="c100 ac"><a href="javascript:'+id+'.cancelUpload('+info.id+');"><img src="../componentes/enviarfotos/ico_limpar.png" width="22" height="22" border="0"></a></div><div class="c30p progress"><div id="progress_'+info.id+'">&nbsp;</div></div><div class="sep"></div>';
	...
	this.op = {
		...
		data:              {id:gal_codigo}, // data object to send with each upload. ex.: { foo: 'bar' }
